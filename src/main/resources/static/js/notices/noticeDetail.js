...
export default class NoticeDetail {
    ...
    async initPage() {
        ...
        // FIXME 관리자하면 수정 버튼 보이게
        this.modalArea.querySelector(".edit-btns").classList.toggle('d-none');
        this.modalArea.querySelector(".delete-btn").classList.toggle('d-none');
    }
    ...
    initEvent() {
        ...
        const deleteBtn = this.modalArea.querySelector('.delete-btn');
        ...
        const cancelBtn = this.modalArea.querySelector('.cancel-btn');
    ...
    async initData() {
        // 데이터 정보 다시 불러와서 다시 그리기
        this.data = await this.parent.getDataById(this.data.id);
    ...
    drawData() {
        ...
        this.modalArea.querySelector('input[name=title]').value = data.title;
        this.modalArea.querySelector('input[name=createdBy]').value = data.createdBy;
        this.modalArea.querySelector('input[name=createdDate]').value = data.createdDate;
    ...
    async saveData() {
        ...
        const response = await fetch(`/notices/${this.data.id}`, {
            method: 'PUT',
            body: formData
        });
